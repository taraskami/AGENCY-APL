...
function mail_search( $QS, $def_date="", $def_type="", $def_selected=false )
{

	$def_date = orr($def_date,dateof("now"));
	$sep = ';';
	$al  = AG_MAIN_OBJECT_DB;
	...
	if (strstr($QS,$sep)) {
		...
		foreach ($searches as $search) {
			...
			if (trim($search)) {
				...
				$x++;
				array_push($results,row(cell(bold("Search #$x (" . trim($search)
						. ")"))) . mail_search(trim($search),$def_date, $def_type));

			}

		}

		return implode(row(cell(hrule(),'colspan="3"')),$results);

	} else {

   		if ( is_numeric( $QS ) ) {

			$filter["$al.".AG_MAIN_OBJECT_DB.'_id'] = $QS;

		} elseif( $x = dateof($QS,"SQL") ) {
		...
		} elseif ( $x = unit_no_of($QS)) {

			$rec = sql_fetch_assoc(get_last_residence($x));

			if ( $x = $rec[AG_MAIN_OBJECT_DB.'_id'] ) {

				$filter["$al.".AG_MAIN_OBJECT_DB.'_id']=$x;

			} else {

				$filter["$al.client_id"]="-99"; // can't match, return nothing

			}

		} else { // regular search $

			$filter["ILIKE:$al.name_full_alias"]="%$QS%";
		}

		$clients = get_clients( $filter );
		$results = show_client_mail_heads( $clients,$def_date,$def_type,(is_numeric($QS) ? true : false) );
